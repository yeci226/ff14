...
export async function getNewsContent(url: string): Promise<{
  blocks: ContentBlock[];
  timestamp?: number;
  avatarUrl?: string;
  publisher?: string;
  ...
  try {
    ...
    const avatarImg = $(".news_info1 .avatar img");
    if (avatarImg.length > 0) {
      const rawSrc = avatarImg.attr("src");
      if (rawSrc) {
        avatarUrl = rawSrc.startsWith("http")
          ? rawSrc
          : `https://www.ffxiv.com.tw${
              rawSrc.startsWith("/") ? "" : "/"
            }${rawSrc}`;
      }
    }
    ...
    article.find("br").replaceWith("__BR_TOKEN__");
    ...
    const processHeading = (level: number, mdPrefix: string) => {
      ...
      if (headings.length > 0) {
        console.log(
          `[Scraper] Found ${headings.length} h${level} tags to convert.`
        );
      }
      headings.each((_, el) => {
        const text = $(el).text().trim();
        console.log(
          `[Scraper] Converting h${level}: "${text}" -> "${mdPrefix}${text}"`
        );
        // Ensure we have newlines around headers
        $(el).replaceWith(`${mdPrefix}${text}`);
      });
    };
    ...
    const extractBlocks = (element: any) => {
      ...
      if (el.type === "tag") {
        if (el.name === "img") {
          ...
          if (src) {
            ...
            if (src.startsWith("data:")) {
              blocks.push({ type: "image", url: src, description: alt });
            } else {
              if (!src.startsWith("http")) {
                // Clean up leading slashes to prevent double slashes if prepending
                const cleanSrc = src.startsWith("/") ? src : "/" + src;
                fullUrl = `https://www.ffxiv.com.tw${cleanSrc}`;
              }
        ...
        // Fix for spacing: <br> or <p><br></p> should be treated as a newline
        // Check if text is effectively empty (only whitespace)
        const textContent = $el.text().replace(/\s/g, "");

        const hasImages = $el.find("img").length > 0;
        // Force recursion for containers to ensure structure is preserved (e.g. <section> with <p><br></p>)
        const isContainer = ["section", "div", "article", "main"].includes(
          el.name
        );
        if (hasImages || isContainer) {
          // Recurse
        ...
        // Implicit spacing for block-level elements
        const isBlockLevel = [
          "p",
          "div",
          "section",
    ...
    // Extract publisher
    const publisher = $(".publisher h2").text().trim();
    ...
    return { blocks, timestamp, avatarUrl, publisher };
  } catch (error) {
...
function cleanText(text: string): string {
  if (!text) return "";
  // Handle special tokens for breaks
  if (text.includes("__BR_TOKEN__")) {
    return text
      .split("__BR_TOKEN__")
